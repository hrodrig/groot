...
# Config schema version (1.0.0+). Omit for legacy pre-1.0 configs; use 1 for new files.
# config_version: 1
...
# kubeconfig cluster name → kube-public/cluster-info → API server host → unknown-cluster.
# Use this for in-cluster pods (no kubeconfig context), e.g. cluster_name: "prod-eks"
cluster_name: ""
...
collection:
  ...
  # When true, per-node host logs (…/proxy/logs/messages → nodes/<node>.log)
  # plus optional kubelet log query (nodes/<node>-kubelet.log). Both are optional jobs.
  include_node_logs: true

  # When >0, adds tailLines to optional kubelet log query; 0 uses API default length.
...
upload:
  ...
  sftp:
    ...
    user: "groot-inbox"
    remote_dir: "inbox"
    # known_hosts_file: "/etc/groot/known_hosts"  # required in production
    # Credentials (identity file) via env — never in YAML:
    #   GROOT_UPLOAD_SFTP_IDENTITY_FILE=/home/groot/.ssh/id_ed25519
`
}
