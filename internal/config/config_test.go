...
func TestSplitSemicolonList(t *testing.T) {
	...
	if got := SplitSemicolonList("a;b"); len(got) != 2 {
...
func TestValidateUploadConfig_sftpWithAllThreeProviders(t *testing.T) {
	cfg := Config{Upload: UploadCfg{
		...
		SFTP:    SFTPUploadCfg{Enabled: true, Host: "h", Port: 22, User: "u"},
	}}
...
func TestValidateUploadConfig_sftpPortRange(t *testing.T) {
	...
	for _, tc := range tests {
		cfg := Config{Upload: UploadCfg{
			Enabled: true,
			SFTP:    SFTPUploadCfg{Enabled: true, Host: "h", Port: tc.port, User: "u"},
		}}
...
func TestValidateConfigVersion(t *testing.T) {
	if err := validateConfigVersion(0); err != nil {
		t.Fatalf("legacy absent: %v", err)
	}
	if err := validateConfigVersion(SupportedConfigVersion); err != nil {
		t.Fatalf("supported: %v", err)
	}
	if err := validateConfigVersion(2); err == nil {
		t.Fatal("expected error for future version")
	}
}

func TestLoad_configVersion(t *testing.T) {
	dir := t.TempDir()
	...
	if cfg.ConfigVersion != 1 {
		t.Fatalf("config_version=%d", cfg.ConfigVersion)
	}
}

func TestValidateUploadConfig_sftpUserRequired(t *testing.T) {
	err := validateUploadConfig(Config{Upload: UploadCfg{
		Enabled: true,
