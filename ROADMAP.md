...
| **Packaging** | **#96** man(1) + nfpm + BSD | Ecosystem | Implemented on `develop` (Unreleased); close GH #2 when tagged. |
...
- ~~**No `man(1)` / nfpm man install**~~ — **#96** / GH #2 implemented on `develop` (Unreleased): `contrib/man/man1/*.1`, nfpm, Homebrew `manpages:`, FreeBSD/OpenBSD ports.
...
| # | Band | Item | Status |
|---|------|------|--------|
| 96 | 4 | 🚀 **man page + nfpm + BSD** ([GH #2](https://github.com/hrodrig/groot/issues/2)): `contrib/man/man1/groot.1` + `kubectl-groot.1`; GoReleaser nfpm + archives; Homebrew `manpages:`; FreeBSD/OpenBSD ports; `make man-sync`. | Done (Unreleased) |
...
| 46 | 4 | 🤝 **Generic `examples/`** beyond profiles (#86): notify/upload/collection skeletons + GKE/AKS profiles; index in `examples/README.md`. | Done (Unreleased) |
