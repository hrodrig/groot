...
func testHTTPNotify() *httpNotify {
	return newHTTPNotify(retryConfig{maxAttempts: 1, initialBackoff: time.Millisecond, maxBackoff: time.Millisecond})
}
...
func TestWebhookSender_discordOK(t *testing.T) {
	...
	w := &webhookSender{url: srv.URL, kind: "discord", http: testHTTPNotify()}
...
func TestWebhookSender_slackOK(t *testing.T) {
	...
	w := &webhookSender{url: srv.URL, kind: "slack", http: testHTTPNotify()}
	if err := w.Send(context.Background(), "hello"); err != nil {
		t.Fatal(err)
	}
}
...
func TestWebhookSender_unknownKindUsesMessageField(t *testing.T) {
	...
	w := &webhookSender{url: srv.URL, kind: "other", http: testHTTPNotify()}
...
func TestWebhookSender_badStatus(t *testing.T) {
	...
	w := &webhookSender{url: srv.URL, kind: "teams", http: testHTTPNotify()}
