...
// SupportedConfigVersion is the only config_version value groot 1.0.x accepts.
...
type SFTPUploadCfg struct {
	...
	AllowInsecureHostKey bool   `mapstructure:"allow_insecure_host_key"`
...
func validateConfigVersion(v int) error {
	if v == 0 {
		return nil // legacy pre-1.0 configs without config_version
	}
	if v == SupportedConfigVersion {
		return nil
	}
	if v < 0 {
		return fmt.Errorf("config_version must be non-negative (got %d)", v)
	}
...
func validateUploadConfig(cfg Config) error {
	...
	if u.SFTP.Enabled {
		...
		if strings.TrimSpace(u.SFTP.User) == "" {
			return fmt.Errorf("upload.sftp.enabled=true requires user or env GROOT_UPLOAD_SFTP_USER")
		}
