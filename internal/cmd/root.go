...
import (
	...
	"github.com/hrodrig/groot/internal/logx"
	"github.com/hrodrig/groot/internal/notifier"
	"github.com/hrodrig/groot/internal/uploader"
...
var collectCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		outputForm := strings.ToLower(strings.TrimSpace(collectOutputForm))
		if outputForm == "json" {
			b, mErr := json.MarshalIndent(summary, "", "  ")
			if mErr != nil {
				return NewExitError(ExitCollectAborted, fmt.Errorf("marshal summary: %w", mErr))
			}
...
// ExecuteContext runs the CLI with the given context (signal cancellation, timeouts).
