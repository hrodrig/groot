...
func TestPostJSONWithRetry_retries5xx(t *testing.T) {
	httpN := newHTTPNotify(retryConfig{maxAttempts: 3, initialBackoff: time.Millisecond, maxBackoff: 5 * time.Millisecond})
	...
	if err := httpN.postJSONWithRetry(context.Background(), srv.URL, []byte(`{}`), nil); err != nil {
		t.Fatal(err)
	}
	if hits != 3 {
...
func TestGenericWebhookSender_bodyTemplateAndHMAC(t *testing.T) {
	...
	w := &genericWebhookSender{
		...
		http:         newHTTPNotify(retryConfig{}),
	}
