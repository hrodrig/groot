...
[![Version](https://img.shields.io/badge/version-1.0.4-blue)](https://github.com/hrodrig/groot/releases)
...
Use a **release tag** instead of `@latest` if you want a pinned version (for example `@v1.0.4`). Documentation for the module: [pkg.go.dev/github.com/hrodrig/groot](https://pkg.go.dev/github.com/hrodrig/groot) (CLI binary; library code is **`internal/`** since **1.0.0**).
...
Pull the image from here (pin to the [version badge](#readme-top) — currently **`1.0.4`**):

```bash
docker pull ghcr.io/hrodrig/groot:1.0.4
```
