...
func (u *sftpUploader) dialSSH(ctx context.Context, host string) (*ssh.Client, error) {
	...
	if knownHostsFile := strings.TrimSpace(u.cfg.KnownHostsFile); knownHostsFile != "" {
		...
		hostKeyCallback = cb
	} else if u.cfg.AllowInsecureHostKey {
		hostKeyCallback = ssh.InsecureIgnoreHostKey()
	} else {
		return nil, fmt.Errorf("known_hosts_file is required (set upload.sftp.known_hosts_file or GROOT_UPLOAD_SFTP_KNOWN_HOSTS; or allow_insecure_host_key: true for testing only)")
	}
